  ...
  return <header className="fixed top-0 left-0 right-0 z-50 bg-background backdrop-blur-md shadow-sm border-b border-border/30">
      <nav className="container-luxury flex items-center justify-between py-5">
        ...
        <div className="hidden lg:flex items-center gap-10">
          {navigation.map(item => <Link key={item.name} to={item.href} className={cn("text-sm font-light tracking-wide transition-colors duration-300", location.pathname === item.href ? "text-foreground" : "text-muted-foreground hover:text-foreground")}>
              {item.name}
            </Link>)}
        </div>
      ...
      backgroundColor: mobileMenuOpen ? '#FAF8F5' : 'transparent'
        ...
        {/* Menu items */}
        <div className="container-luxury py-10 flex flex-col gap-8 bg-primary-foreground">
          {navigation.map((item, index) => <Link key={item.name} to={item.href} onClick={() => setMobileMenuOpen(false)} className={cn("text-2xl font-serif font-medium transition-all duration-300", location.pathname === item.href ? "text-foreground" : "text-muted-foreground", mobileMenuOpen && "animate-fade-in-up")} style={{
          animationDelay: `${index * 0.1}s`
        }}>
              {item.name}
            </Link>)}
          <Link to="/contact" onClick={() => setMobileMenuOpen(false)} className="mt-4 text-center text-sm font-light tracking-wide border border-foreground/20 px-6 py-4 rounded-sm hover:bg-foreground hover:text-primary-foreground transition-all duration-300">
            Enquire Now
          </Link>
        </div>
      </div>
